    ...
    */
    ...
    impl PartialEq for BoundedPNCounterv2 {
        fn eq(&self, other: &Self) -> bool {
            self.positive_count == other.positive_count && self.negative_count == other.negative_count
        }
    }
    ...
    impl ShoppingList{
        ...
        pub fn add_or_update_item(&mut self, item_name: String, quantity_change: i32){
            ...
            if let Some(existing_item) = self.items.get_mut(&item_name) {
                ...
                else{ // if Item do not exist, and someone want , just add the item with no increment/decrement values
...
mod property_shopping_list_tests {
    ...
    fn shopping_list_strategy() -> impl Strategy<Value = ShoppingList> {
        ...
        proptest::collection::hash_map(item_strategy, any::<i32>(), 0..100)
            .prop_map(|items| {
